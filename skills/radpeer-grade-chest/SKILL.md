---
name: radpeer-grade-chest
description: Compare two CT chest radiology reports at the finding level using RADPEER-adapted grading
---

Compare **Report A** against **Report B** for a **CT chest** study at the finding
level and grade each discrepancy.

This skill is the chest variant of the `radpeer-grade` family. The detailed
clinical rules (Fleischner, Lung-RADS, lung-cancer screening, mediastinal
findings, coronary calcium, pleural disease, etc.) are loaded as a knowledge
block by the caller before this prompt — apply them as authoritative.
...
a pulmonary nodule, an effusion, a calcification, a mediastinal node, a normal
structure explicitly noted, a recommendation, etc.
...
imaging, biopsy, referral, or alters treatment.
...
- Pulmonary nodules: characterize by solid/part-solid/ground-glass and size in
  mm. Apply Fleischner / Lung-RADS thresholds from the knowledge block.
- Coronary calcium: mandatory mention on lung-screening protocols — omission
  is a clinically meaningful gap, not stylistic.
- Emphysema on lung-screening exams: presence and severity must be reported.
- Mediastinal lymph nodes: short-axis ≥10 mm is a positive finding.
- Pleural effusion characterization (trace / small / moderate / large) is a
  clinical descriptor — different volume categories are grade 2b at minimum.
- Incidental abdominal findings (upper abdomen visible on chest CT) — grade
  them using abdomen rules if the knowledge block loads them.
